...
export async function pingProgram(operation: number, operatingValue: number) {
    ...
    const calcInstructions = await createCalculatorInstructions(operation, operatingValue);
    ...
    const instruction = new TransactionInstruction ({
        ...
        programId,
        data: calcInstructions // Our calculator instruction we generated 5 lines above
    });
...
export async function example(programName: string, accountSpaceSize: number) {
    ...
    await pingProgram(4, 3); // Devide by 3
}
